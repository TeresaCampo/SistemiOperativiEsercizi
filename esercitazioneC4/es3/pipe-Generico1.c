...
int L;				/*lunghezza della stringa letta, senza terminatore*/
int pidFiglio, status, ritorno;      /* per wait padre */
	...
	{   
		...
		printf("DEBUG-Figlio %d sta per iniziare a scrivere una serie di messaggi, ognuno di lunghezza variabile, sulla pipe dopo averli letti dal file passato come parametro\n", getpid());
		...
		{   	if(mess[L]=='\n'){
			...
			j++;	L=0;
			}	/*mando al padre il numero di caratteri e poi il messaggio */
			else{
		    		++L;}
		}
	...
	while (read(piped[0], &L, sizeof(int)))  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 	read(piped[0], mess, L);
